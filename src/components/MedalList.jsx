...
import { TableGroup, MedalTable, MedalTableCell, SortCheck } from "../styles/MedalTableStyle";
...
function MedalList({ countries, setCountries }) {
    ...
    const getSortedCountries = () => {
        if (sortType === 'gold') {
            ...
            const sortedCountries = [...countries].sort((a, b) => (b.gold + b.silver + b.bronze) -
            ...
            <TableGroup>
                {countries.length === 0 ? (
                    ...
                    <>
                        <SortCheck>
                            <label>
                                <input type="radio" value="gold" checked={sortType === 'gold'}
                                    onChange={(e) => setSortType(e.target.value)} />
                                금메달 순
                            </label><label>
                                <input type="radio" value="total" checked={sortType === 'total'}
                                    onChange={(e) => setSortType(e.target.value)} />
                                합계 순
                            </label>
                        </SortCheck>
                        ...
                        {sortedCountries.map((country, index) => {
                            return (
                                <MedalTable key={index} className={index % 2 === 0 ? 'even' : 'odd'}>
                                    ...
                                    <MedalTableCell>{country.gold + country.silver + country.bronze}</MedalTableCell>
                                    <MedalTableCell>
                                        <Button type='button'
